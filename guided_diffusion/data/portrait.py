...
class Portrait(Dataset):
    def __init__(self, root, size = 128, base_res = 128, flip=False,
        ...
        self.lmk_path = f'{root}/landmark/landmark_128.pkl'
        self.landmarks = pickle.load(open(self.lmk_path,'rb'))
        
        self.src_list = os.listdir(f'{root}/align/source')
        self.src_list = [x for x in self.src_list if x not in self.error_img['source']]
        self.src_list.sort()
        self.tgt_list = os.listdir(f'{root}/align/target')
        ...
        print(f"len(src) + len(tgt) == {len(self.src_list) + len(self.tgt_list)}")
    ...
    def __getitem__(self, index): # index: 0 - 9350
        ...
        src_index = index if index < len(self.src_list) else index - len(self.tgt_list) - 1 # 0 - 8
        # tgt_index = index - len(self.tgt_list) * src_index # 0 - 1038
        tgt_index = index if index < len(self.tgt_list) else index - len(self.tgt_list) - 1
        batch = {}
        # print(f"index: {index}")
    ...
    def extract_convex_hulls(self, landmark):
        ...
        mask_dict['mask'] = np.expand_dims(mask_dict['mask'], axis=0)
        return mask_dict
